...
impl From<RawHeatpump> for Heatpump {
    fn from(input: RawHeatpump) -> Self {
        Self {
            ...
            heat: Energy::new::<watt_hour>(input.heat_wh as f64),
            cold: Energy::new::<watt_hour>(input.cold_wh as f64),
            defrost: Energy::new::<watt_hour>(input.defrost_wh as f64),
            cop: Ratio::new::<percent>(input.cop_pct as f64),
            boiler_top: input
                .boiler_top_degc_e1
...
impl TryFrom<&Heatpump> for RawHeatpump {
    ...
    fn try_from(input: &Heatpump) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            heat_wh: input.heat.get::<watt_hour>().round() as i64,
            cold_wh: input.cold.get::<watt_hour>().round() as i64,
            defrost_wh: input.defrost.get::<watt_hour>().round() as i64,
            cop_pct: input.cop.get::<percent>().round() as i16,
